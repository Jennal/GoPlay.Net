...
public class Excel2ProtoEnum
{
    public static Dictionary<string, List<(string, int)>> Enums = new Dictionary<string, List<(string, int)>>();
    
    private static string CLASS_TEMPLETE = GeneratorUtils.GetTpl("tpl_class_proto_enum");
...
//                            Debug.Log($"{xls} => {name}");   
    ...
    static TemplateEnumData ConvertToEnum(string enumFolder, string xls, ExcelWorksheet table, bool isDryRun, string tpl)
    {
        var tableName = table.Name.Substring(ExporterConsts.exportPrefix.Length);
        if (string.IsNullOrEmpty(tableName))
        {
            ExporterUtils.Error($"[错误]表名不存在：{xls} => {table.Name}");
            return new TemplateEnumData();
        }

        var rowColumns = ExporterUtils.GetRowColumn(table);
        return ConvertTable(enumFolder, xls, table, rowColumns, isDryRun, tpl);
    }

    static TemplateEnumData ConvertTable(string enumFolder, string xls, ExcelWorksheet table, Vector2Int rowColumn, bool isDryRun, string tpl)
    {
        var tableName = table.Name.Substring(ExporterConsts.exportPrefix.Length);
        if (rowColumn.x <= 0 || rowColumn.y <= 0) return new TemplateEnumData();
    ...
    static string WriteEntityFile(string enumFolder, string entityName, string content)
    {
        ...
        var path = Path.Combine(enumFolder, ExporterConsts.enumFolder, entityName + ".proto");
