...
public class Excel2Json
{
    ...
    private static void Export(string xls, ExcelWorksheet table, ExcelPackage package, DataExportPlan plan)
    {
        ...
        var outPath = Path.Combine(_outFolder, ExporterConsts.dataFolder, variantName, typeName + ".json");

        //多个Excel中的同名Sheet，按文件名顺序追加到同一个数据文件
    ...
    private static void SaveAsset(ConfValues asset, ExcelWorksheet table, string outPath)
    {
        ...
        var content = GeneratorUtils.RenderTpl(DATA_TEMPLETE, new {data = json});
