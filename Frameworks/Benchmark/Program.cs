...
namespace GoPlay.Benchmarks
{
    public class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            //   dotnet run -c Release -- concurrency  # BenchmarkDotNet 跑 Concurrency × Delay 矩阵
            //   dotnet run -c Release -- route        # 只跑 BenchmarkRoute（反射 vs 编译委托）
            //   dotnet run -c Release -- request      # 只跑 BenchmarkRequest（空业务 Echo）
            ...
            switch (mode)
            {
                ...
                case "all":
                    ...
                    break;
                default:
                    Console.WriteLine($"Unknown mode: {mode}. Valid: report | concurrency | route | request | all");
                    break;
            }
        }
    }
