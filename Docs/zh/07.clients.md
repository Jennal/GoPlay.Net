...
> English version: [07.clients.md](../en/07.clients.md)

GoPlay 官方维护 3 种客户端：C#（桌面、Unity）、TypeScript（Web / Node / 小程序）、JavaScript（免构建的纯浏览器）。三端遵循同一份 wire protocol（详见 [10.protocol.md](./10.protocol.md)），Handshake / Heartbeat / Request / Notify / Push 语义一致。
...
- Request / Response 靠 `Id` 对齐，详见 [10.protocol.md](./10.protocol.md)。
