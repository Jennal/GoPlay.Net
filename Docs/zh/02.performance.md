...
> English version: [02.performance.md](../en/02.performance.md)
>
...
- **Graceful shutdown 有预算**：Server 关闭会等所有 in-flight 请求和客户端 Disconnect 事件链 drain 完，但有 2s（Sender） + 10s（整体）硬上限，避免 k8s `terminationGracePeriodSeconds` 到点 SIGKILL。见 [08.advanced.md](./08.advanced.md#优雅停机)。
